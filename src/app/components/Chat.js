...
const Chat = (props) => {
  ...
  var setConversation = props.setConversation;
  var showEmptyChat = props.showEmptyChat;
  var setShowEmptyChat = props.setShowEmptyChat;
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        setConversation(data.formattedMessages);
        setShowEmptyChat(data.formattedMessages.length === 0);
      } catch (error) {
        console.error(error);
  ...
  const sendMessage = async (e) => {
    ...
    const userMessage = { content: message, role: 'user', byUser: session.user.email };
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const systemMessage = { content: data.message, role: 'system', byUser: session.user.email  };
        ...
        // Save messages
        await fetch('/api/message/saveMessages', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <div className="flex max-w-full flex-1 flex-col">
        <div className="relative h-full w-full transition-width flex flex-col overflow-hidden items-stretch flex-1">
          <div className="flex-1 overflow-hidden relative" >
            <div className="react-scroll-to-bottom--css-ikyem-79elbk h-full dark:bg-gray-800 overflow-auto">
              <div className="react-scroll-to-bottom--css-ikyem-1n7m0yu h-full">
                {!showEmptyChat && conversation.length > 0 ? (
                  <div className="flex flex-col items-center text-sm bg-gray-800 ">
                    <div style={{position:"sticky",top:0}} className="z-50 flex w-full items-center justify-center gap-1 border-b border-black/10 bg-gray-50 p-3 text-gray-500 dark:border-gray-900/50 dark:bg-gray-700 dark:text-gray-300 ">
